  ...
  const mouseClicked = (p5) => {
    try {
      ...
      if (canvasX >= 0 && canvasX < p5.width && canvasY >= 0 && canvasY < p5.height) {
        ...
        // Rest of your code for handling clicks within the canvas
        ...
        // Rest of your code for updating the board and dispatching actions
        if (player) {
            dispatch(updateBoard({ row: row, col: col, value: 1 })); // Update the clicked cell to 1 
            setPlayer(!player);
    
            const capturedPieces = checkSurroundedPieces(board, col, row, 1);
            capturedPieces.forEach(({ x, y }) => {
              dispatch(updateBoard({ row: row, col: col, value: 0 })); // Remove captured pieces by setting their value to 0
            });
        } else {
            dispatch(updateBoard({ row: row, col: col, value: 2 })); // Update the clicked cell to 2
            setPlayer(!player);
    ...
    try {
      ...
      const checkSurroundedNeighbors = (x, y) => {
        ...
        const checkNeighbors = (nx, ny) => {
          console.log("checkNeighbors called: " + "nx=" + nx + ny);
          if (
            nx >= 0 &&
            nx < cols &&
            ny >= 0 &&
